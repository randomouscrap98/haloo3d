...
int octree_init(octree * tree) {
  tree->model = NULL;
  tree->facecache = NULL;
  int result = vector_ocfindex_init(&tree->faces);
  if(result) goto END;
...
ERROR1:
  vector_ocfindex_free(&tree->faces);
END:
  return result;
}
...
int octree_recurse(octree* tree, size_t parentidx, vector_ocfindex * parentfaces, uint32_t depth) {
  // First, calculate which tris are in this octree node. 
  vector_ocfindex nodefaces;
  int result = vector_ocfindex_init(&nodefaces);
  if(result) {
  ...
  if(depth == 0) {
    // A pure copy
    result = vector_ocfindex_append(&nodefaces, parentfaces);
    if(result) {
    ...
    for(size_t i = 0; i < parentfaces->length; i++) {
      if(AABBCOLLIDE_3DBOX(parentfaces->array[i]->min, parentfaces->array[i]->max, 
        ...
        result = vector_ocfindex_push(&nodefaces, &parentfaces->array[i]);
        if(result) {
  ...
  if(nodefaces.length > 2 * depth) {
    ...
    result = vector_ocfindex_append(&tree->faces, &nodefaces);
    if(result) {
...
ERROR1:
  vector_ocfindex_free(&nodefaces);
END:
  return 0;
}
...
static int octree_build_init(octree * tree, uint32_t numfaces, size_t * rootidx) {
  ...
  vector_ocfindex_clear(&tree->faces);
  NULLFREE(tree->facecache);
...
int octree_build(octree * tree, h3d_obj * model) {
  ...
  vector_ocfindex roottris;
  result = vector_ocfindex_init(&roottris);
  if(result) {
