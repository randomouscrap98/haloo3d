...
static inline void vec3_subtract(float_t *v0, float_t *v1, float_t *out) {
  out[0] = v0[0] - v1[0];
  out[1] = v0[1] - v1[1];
  out[2] = v0[2] - v1[2];
}
...
static inline void vec3_normalize(float_t *v0, float_t *out) {
...
static inline void vec3_cross(float_t *v0, float_t *v1, float_t *out) {
...
static inline void mat4_zero(float_t *result) {
  result[0] = H3DVF(0.0);
  result[1] = H3DVF(0.0);
  result[2] = H3DVF(0.0);
  result[3] = H3DVF(0.0);
...
static inline void mat4_identity(float_t *result) {
  result[0] = H3DVF(1.0);
  result[1] = H3DVF(0.0);
  result[2] = H3DVF(0.0);
  result[3] = H3DVF(0.0);
...
static inline void h3d_vec4_mult_mat4(float_t *v, float_t *m, float_t *out) {
...
static inline void h3d_mat4_prescale_self(float_t *m, float_t *scale) {
  m[0] *= scale[0];
  m[1] *= scale[0];
  m[2] *= scale[0];
  m[3] *= scale[0];
...
static inline void h3d_vec4_homogenous(float_t *v) {
  if (v[H3DW] != H3DVF(1)) {
...
// Calculate the look vector (last param0, using it to set the given matrix view
// to the "look_at" matrix
void h3d_camera_calclook(h3d_camera *cam, float_t *view, float_t *lookvec);

// Calculate movement based only on yaw (pretty normal)
void h3d_camera_calcmove_yaw(h3d_camera *cam, float_t *delta);
